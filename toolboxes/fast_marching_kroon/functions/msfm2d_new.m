...
% TMH: Uncommented next line. Otherwise, an error was introced unless
% 1/F was an integer!!
...
if UseType==2;
    % TRYING TO MANUALLY START THE PROPAGATION
    SourcePoints_org=SourcePoints;
    %nes =[-1 -1; 1 1; -1 1; 1 -1;
    nes=[    -1 0;
        1 0;
        0 -1;
    ...
    nes=[0 0]
    is=0;
    for i=1:size(SourcePoints_org,2);
        x=SourcePoints_org(1,i);
        y=SourcePoints_org(2,i);
        for j=1:size(nes,1);
            ix=nes(j,1);iy=nes(j,2);
            d=sqrt(ix^2+iy^2);
            t=0.5*d/F(x,y) + 0.5*d/F(x+ix,y+iy);
            
            
            is=is+1;
            SourcePoints(:,is)=[x+ix y+iy]';
    ...
    for z=1:size(SourcePoints,2)
        ...
        %Frozen(x+1,y)=0;T(x+1,y)= 0.5/F(x,y) + 0.5/F(x+1,y);
        %Frozen(x-1,y)=0;T(x-1,y)= 0.5/F(x,y) + 0.5/F(x-1,y);
        %Frozen(x,y+1)=0;T(x,y+1)= 0.5/F(x,y) + 0.5/F(x,y+1);
        %Frozen(x,y-1)=0;T(x,y-1)= 0.5/F(x,y) + 0.5/F(x,y-1);
        
        %Frozen(x+1,y+1)=1;T(x+1,y+1)= 0.5*sqrt(2)/F(x,y) + 0.5*sqrt(2)/F(x+1,y+1);
        %Frozen(x+1,y-1)=1;T(x+1,y-1)= 0.5*sqrt(2)/F(x,y) + 0.5*sqrt(2)/F(x+1,y-1);
        %Frozen(x-1,y+1)=1;T(x-1,y+1)= 0.5*sqrt(2)/F(x,y) + 0.5*sqrt(2)/F(x-1,y+1);
        %Frozen(x-1,y-1)=1;T(x-1,y-1)= 0.5*sqrt(2)/F(x,y) + 0.5*sqrt(2)/F(x-1,y-1);
...
disp(sprintf('%02d (i,j)=(%d,%d)',0,x,y))
disp( [T(18:22,18:22),Frozen(18:22,18:22)] )
neg_list(:,1:neg_pos)
%pause(1)
%keyboard
...
for itt=1:numel(F)
    ...
    neg_list(:,neg_pos)=0; % %% ADDED TO MAKE THE CODE EASIER TO DEBUG
    neg_pos =neg_pos-1;
    ...
    if itt<20
        %    disp(sprintf('%04d -- T(19,19)=%g',itt,T(19,19)))
        disp(sprintf('%02d-(i,j)=(%d,%d)',itt,x,y))
        disp( [T(18:22,18:22),Frozen(18:22,18:22)] )
        %pause(.1)
    end
    neg_list(:,1:neg_pos)
