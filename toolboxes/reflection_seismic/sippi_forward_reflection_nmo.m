...
for ip=1:length(prior)
    ...
    elseif strcmp(lower(prior{ip}.name),'vpvsrho');
        ...
        vs=vs(:);
        rho=rho(:);
    elseif strcmp(lower(prior{ip}.name),'vprho');
        % Cholesky type prior / split vp,rho from 1x[2*nm] vector
        nt=length(prior{ip}.y);
        it=1:nt;
        vp=m{ip}(it);
        rho=m{ip}(it+nt);
...
%% CHECK FOR VS
if ~exist('vs','var')
    if ~isfield(prior{ip},'vpvs');
        prior{ip}.vpvs=sqrt(2);
    end
    vs=vp./vpvs;
end
