function [dt nt error addpar]=fwi_execute(Ncores_applied,ant_pos,sim_mode,addpar)
...
for i=1:ceil(Ntrn/Ncores_applied)
    ...
    for k=1:Ncores_applied
        ...
        ntrn=1;
        Fid=fopen(sprintf('%s%s%s',fpath,filesep,'trnrec.cor'),'wt');
        try
        ...
        if addpar.first==1 % The following only needs to be written in the first itereation:
            % Delete old trace files:
            delete(sprintf('%s%s%s',fpath,filesep,'*.dat'))
            
            % Setting up the batch-file:
            % First delete the old batch file:
            if isunix
              batch_file=sprintf('%s%s%s',fpath,filesep,'forward.sh');
            else
              batch_file=sprintf('%s%s%s',fpath,filesep,'forward.bat');
            end
            ...
            if co==1 && sn==0 && fr==0
                fprintf(fid,'%s %s%i\n',addpar.executable,'/c:',coord);
            end
            if co==1 && sn==1 && fr==0
                fprintf(fid,'%s %s%i %s%i\n',addpar.executable,'/c:',coord,'/s:',snap);
            end
            if co==1 && sn==1 && fr==1
                fprintf(fid,'%s %s%i %s%i %s%i\n',addpar.executable,'/c:',coord,'/s:',snap,'/f:',frac);
            end
            if co==0 && sn==1 && fr==0
                fprintf(fid,'%s %s%i\n',addpar.executable,'/s:',snap);
            end
            if co==0 && sn==1 && fr==1
                fprintf(fid,'%s %s%i %s%i\n',addpar.executable,'/s:',snap,'/f:',frac);
            end
            if k<Ncores
                fprintf(fid,'%s','exit');
            end
        ...
        % Execute the forward FDTD simulation through the batch_file  
        if k<Ncores_applied
          %cmd=sprintf('START /B %s%s%s',fpath,filesep,'forward.bat')
          if isunix
            s1=system(['sh ',batch_file,' &']);
          else
            s1=dos(['START /B ',batch_file]);
          end
        else
          if isunix
            s2=system(['sh ',batch_file]);
          else
            s2=dos(batch_file);
          end
        end
        ...
        if s1==-1 || s2==-1
            disp('The external executable did not start -1')
            error=1;
            return
        end
    end
    ...
    for j=1:Ncores_applied
        ...
        WorkDir=[addpar.workdir,filesep,sprintf('run%03d',j)];
        fpath=WorkDir;
        ...
        while wait==1
            try
                Fid=fopen(sprintf('%s%s%s',fpath,filesep,'trace_out000.dat'),'rb+');
...
%            Residual_Ez=d_obs.Trace_Ez-(Trace_Ez_resid'*diag(faktor))';
            ...
            save2Dbin(sprintf('%s%s%s',fpath,filesep,'resid00.E'),nx,nz,Residual_Ez');
            ...
            Fid=fopen(sprintf('%s%s',fpath,filesep,'resid00.N'),'wb');
...
min_time=0.1;
if time<min_time;
    disp(sprintf('The external executable did not start (time spent is less than %g seconds',time))
    error=1;
    return
end
