...
for im=im_array;
    ...
    elseif (strcmp(prior{im}.type,'FFTMA'))       
        ...
        % UPDATE COVARIANCE PARAMETERS IF THE EXIST
        ...
        %prior=sippi_prior_init(prior);
        %if ~isfield(prior{im}.FFTMA,'options');prior{im}.FFTMA.options.null='';end
        ...
        %[m_propose{im},prior{im}.fftma_options.z_rand,o]=fft_ma(prior{im}.x,prior{im}.y,prior{im}.Va,prior{im}.fftma_options);
        %else
        %    [m_propose{im},prior{im}.fftma_options.z_rand,o]=fft_ma(prior{im}.x,prior{im}.y,prior{im}.z,prior{im}.Va,prior{im}.fftma_options);
        %end
        
        [m_propose{im},z_rand,prior{im}.fftma_options]=fft_ma(prior{im}.x,prior{im}.y,prior{im}.Va,prior{im}.fftma_options);
        prior{im}.fftma_options.z_rand=z_rand;
        
        
        % IF NO CHANGE IN COVARIANCE
        if prior{im}.fftma_options.constant_C~=1;
            prior{im}.fftma_options=rmfield(prior{im}.fftma_options,'fftC');
        end
        
        
        %        % IF NO CHANGE IN COVARIANCE
        %        if prior{im}.fftma_options.constant_C==1
        %            prior{im}.fftma_options.fftC=o.fftC;
        %            %prior{im}.fftma_options.nf=o.nf;
        %        end
...
for im=1:nm;
    if (isfield(prior{im},'run'))
        ...
        if strcmp(prior{im}.type,'FFTMA')
            ...
            % update Va parameters from parameters
            
            %disp('RUN FFTMA');
            % RUN FFTMA WITHOUT ANY PERTUBATION OF RANDOM PARAMETERS
            % make sure random numbers are not perturbed
            fftma_options=prior{im}.fftma_options;
            fftma_options=rmfield(fftma_options,'lim');
            if length(prior{im}.z)==1
            ...
            % NEXT LINE MAYBE AT THE EN AS A GENERAL TRANSF
            if isfield(prior{im},'o_nscore');
                Va_par=deformat_variogram(prior{im}.Va);
                gvar=sum([Va_par.par1]);
                m_propose{im}=m_propose{im}./sqrt(gvar);
                m_propose{im}=inscore(m_propose{im},prior{im}.o_nscore);
            else
...
%% PERHAPS IF PERTURBED MODEL IS IDENTIAL TO THE UNPERTURBED MODEL
